...
proc hotplugCallback(ctx: ptr LibusbContext; device: ptr LibusbDevice;
  ...
  if rc != (cint)LibusbError.success:
    ...
    echo "Device attached: vendor 0x", toHex(desc.idVendor, 4), ", product 0x", toHex(desc.idProduct, 4)
  else:
    echo "Device detached: vendor 0x", toHex(desc.idVendor, 4), ", product 0x", toHex(desc.idProduct, 4)
  inc done
  result = 0
